...
# Paradise Reclaimed: the only owner of scene transitions.
# Ported from clvi.cc/Clean Ascension/game/src/app/SceneRouter.gd with three fixes:
#   * emit_signal(), not the Godot 4 `.emit()`
#   * no return type on _load_scene_async (a coroutine yields GDScriptFunctionState)
#   * world_scene now resolves -- World_Main.tscn did not exist in the source
...
func _load_scene_async(scene_path):
...
func _get_fade_layer() -> Node:
	if fade_layer_path == null or str(fade_layer_path) == "":
		return null
	if not has_node(fade_layer_path):
		return null
